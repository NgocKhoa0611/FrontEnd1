          ...
          {currentProducts && currentProducts.length > 0 ? (
            currentProducts.map(detailItem => (
              <tr key={detailItem.product_detail_id}>
                ...
                <td>
                  <Link to={`/admin/productdetaillist/edit-detail/${detailItem.product_detail_id}`} className="edit-btn-prod"> Sửa</Link>
                  <button className={`hide-btn-products ${detailItem.is_hidden ? 'show-text' : ''}`} onClick={() => hideProduct(detailItem.product_detail_id, detailItem.is_hidden)}>
